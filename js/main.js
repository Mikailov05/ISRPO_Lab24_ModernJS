...
function step2(predsResult) {
    return new Promise((resolve) => {
        setTimeout(() => resolve(`${predsResult} -> Шаг 2 завершён`), 500);
    });
}

function step3(predsResult) {
    return new Promise((resolve) => {
        setTimeout(() => resolve(`${predsResult} -> Шаг 3 завершён`), 500);
    });
}
...
async function processOrder() {
    try {
        const checkStock = await new Promise((resolve) => {
            setTimeout(() => {
        ...
        const calculatePrice = await new Promise((resolve) => {
            setTimeout(() => {
                resolve("Стоимость: 5000 руб.");
            }, 500);
        });
        console.log(calculatePrice);

        const confirmOrder = await new Promise((resolve) => {
            setTimeout(() => {
